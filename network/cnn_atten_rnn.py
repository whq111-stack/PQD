...
# 原始 LGAN 网络定义。
# 输入是 batch × 1 × 640 的一维 PQD 波形，输出是 8 维多标签概率。
class CNN(nn.Module):
    """
...
class cnn_atten_rnn(nn.Module):
    ...
    def forward(self, x, label_features):
        """输入一维波形和 one-hot 标签特征，输出 8 维 sigmoid 概率。"""
        x = self.cnn(x)
