...
# BPMLL 和 CNN-BPMLL 的 Keras 基线训练脚本。
# 它与 PyTorch LGAN 使用不同的损失和模型接口，主要用于论文对比。
#save and show loss&acc
class LossHistory(Callback):
    """记录训练/验证曲线，并保存验证准确率最高的 Keras 模型。"""
...
def cnn_bpmll(input_dim=640, hidden_dim=128, output_dim=8):
    """构造 CNN 特征提取器与 BPMLL 分类头组合的基线网络。"""
    """
    model=creat_model(input_dim,hidden_dim=128)
    """
...
def multi_acc(y_true, y_pred):
    """计算 Keras 版本的 exact-match 多标签准确率。"""
